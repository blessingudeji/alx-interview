#!/usr/bin/python3
""" Script that solves the N queens challenge"""

import sys


def is_safe(board, row, col):
    """Determine if there is a queen in the same column
    """
    for i in range(row):
        if board[i] == col:
            return False

    # Determine if there is a queen on the upper diagonal
    for i, j in zip(range(row-1, -1, -1), range(col-1, -1, -1)):
        if board[i] == j:
            return False

    # Determine if there is a queen on the lower diagonal
